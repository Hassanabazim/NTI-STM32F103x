...
ErrorState_t MRCC_enSysClkINIT(void)
{
	...
	while (!(GET_BIT(RCC->RCC_CR, HSE_RDY)));
	...
	while (!(GET_BIT(RCC->RCC_CR, HSI_RDY)));
	...
	CLR_BIT(RCC->RCC_CFGR, PLL_SRC);

#elif (RCC_PLL_INPUT_MODE == RCC_PLL_HSE)
	SET_BIT(RCC->RCC_CFGR, PLL_SRC);
	CLR_BIT(RCC->RCC_CFGR, PLL_XTPRE);

#elif (RCC_PLL_INPUT_MODE == RCC_PLL_HSE_DIV_2)
	SET_BIT(RCC->RCC_CFGR, PLL_XTPRE);
#else
#error("WRONG PLL INPUT SELECTION!");
	...
	while (!(GET_BIT(RCC->RCC_CR, PLL_RDY)));
