...
extern MNVIC_CFG_t IRQ_PORT[MNVIC_IRQ_NUM];
...
ErrorState_t MNVIC_enIRQInit(void)
{
	...
	for (u8 i = 0; i < MNVIC_IRQ_NUM; i++)
	{
		if (IRQ_PORT[i].IRQ_State == ENABLE)
		{
			if (IRQ_PORT[i].IRQnum < MNVIC_MAX_REG1)
			{
				NVIC->NVIC_ISER[0] = (1<<IRQ_PORT[i].IRQnum );
			}
			else if (IRQ_PORT[i].IRQnum  < MNVIC_MAX_REG2)
			{
				IRQ_PORT[i].IRQnum  -= MNVIC_MAX_REG1;
				NVIC->NVIC_ISER[1] = (1<<IRQ_PORT[i].IRQnum );
			}
			else
			{
...
ErrorState_t MNVIC_enIRQDeInit(void)
{
	...
	SCB_AIRCR = MNVIC_GROUP_16_SUB_0;

	for (u8 i = 0; i < MNVIC_IRQ_NUM; i++)
	{
		if (IRQ_PORT[i].IRQnum  < MNVIC_MAX_REG1)
		{
			NVIC->NVIC_ICER[0] = (1<<IRQ_PORT[i].IRQnum );
		}
		else if (IRQ_PORT[i].IRQnum  < MNVIC_MAX_REG2)
		{
			IRQ_PORT[i].IRQnum -= MNVIC_MAX_REG1;
			NVIC->NVIC_ICER[1] = (1<<IRQ_PORT[i].IRQnum );
		}
		else
		{
			local_state = OUT_OF_RANG_ERR;
